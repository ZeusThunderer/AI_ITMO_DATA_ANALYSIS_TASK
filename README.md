...
_Этот график показывает, насколько несбалансированы классы. Это важно для выбора метрик и подходов к обучению моделей._

**2. Временной анализ**

![temporal_analysis](./eda/plots/temporal_analysis.png)
_Здесь видно, в какие дни недели и часы чаще всего происходят мошенничества. Это помогает строить гипотезы о ночных атаках или активности в выходные._
...
_Показывает, какие признаки сильнее всего связаны с мошенничеством. Это помогает выбрать, на что делать упор при построении модели._
...
- Explainable AI (объяснимость решений для пользователей и регуляторов)

В файле `technical_hypotheses.md` я подробно расписал каждую гипотезу и объяснил, зачем она нужна.

---

## Пояснения к техническим гипотезам (простым языком)

- **Машинное обучение** — используем современные алгоритмы, чтобы находить сложные схемы мошенничества, которые не описать простыми правилами.
- **Feature Engineering** — придумываем новые признаки (например, частота операций, география), чтобы модель лучше понимала поведение клиентов.
- **Real-time Detection** — система должна работать быстро, чтобы блокировать мошенничество до завершения транзакции.
- **Ensemble Methods** — объединяем несколько моделей, чтобы повысить точность и устойчивость.
- **Anomaly Detection** — ищем аномалии, чтобы ловить новые, еще неизвестные схемы мошенничества.
- **Explainable AI** — важно объяснять, почему система считает транзакцию подозрительной, чтобы пользователи и регуляторы доверяли результатам.
...
- Средняя экономия на одной предотвращенной мошеннической транзакции — $414.83
- Если внедрить систему, можно сэкономить миллионы долларов
- Можно сфокусировать проверки на самых рискованных странах и валютах, а для низкорисковых — автоматизировать
- Анализ помогает строить стратегию развития и защищать бизнес от новых угроз
